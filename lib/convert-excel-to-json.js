...
const excelToJson = (function() {
    ...
    const parseSheet = (sheetData, workbook) => {
        ...
        // include empty cells
        if (_config.includeEmptyCells) {
            rows = rows.reduce((result, row) => {
                const processedRow = includeEmptyCells(row);
    ...
    const includeEmptyCells = (obj) => {
        const keys = Object.keys(obj);
        let lastDefinedIndex = keys.length - 1;
        ...
        if (allValuesUndefined) {
          return null; // or return nothing (undefined)
        }
    ...
    const convertExcelToJson = function(config = {}, sourceFile) {
        ...
        // ignoring empty cells by default
        _config.includeEmptyCells = _config.includeEmptyCells || false;
